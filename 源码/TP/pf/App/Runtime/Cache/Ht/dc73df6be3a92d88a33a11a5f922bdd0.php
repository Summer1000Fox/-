                ...
                <div class="col-lg-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            轮播图列表 : 筛选出符合条件的<?php echo ($slideshowcount); ?>条记录
                        </div>
                        <div class="panel-body">
                        	<div class="well">
								<div class="head"><span>搜索轮播图</span></div>
								<div class="main">
									<form id="form_search" method="get" action="<?php echo U(slideshow);?>">
										<table class="table">
											<tr>
												<td>
													<input type="text" name="title"  placeholder="轮播图名字" class="form-control" />
												</td>
												<td>
													<input type="submit" value="搜索轮播图名字" class="btn btn-primary" />
												</td>
											</tr>
										</table>
									</form>
							...
							<table class="table table-striped table-bordered table-hover">
								<tr>
									<td colspan="16">
										<a href="<?php echo U(addslideshow);?>">[添加轮播图]</a>
								...
								<?php if(!empty($slideshowlist)): if(is_array($slideshowlist)): foreach($slideshowlist as $key=>$slideshowlistval): ?><tr>
									<td><?php echo ($slideshowlistval["rid"]); ?></td>
									<td><?php echo ($slideshowlistval["title"]); ?></td>
									<td><a href="/TP/pf/Public<?php echo ($slideshowlistval["pic"]); ?>"><img width="120px" height="50px" src="/TP/pf/Public<?php echo ($slideshowlistval["pic"]); ?>" alt="<?php echo ($slideshowlistvalpic["pic"]); ?>"></a></td>
									<td><?php echo ($slideshowlistval["adminid"]); ?></td>
									<td><?php echo ($slideshowlistval["priority"]); ?></td>
									<td><?php echo ($slideshowlistval["rcode"]); ?></td>
									<td><?php echo ($slideshowlistval["url"]); ?></td>
									<td>
										<a href="<?php echo U('editslideshow',array('id'=>$slideshowlistval[rid],'adminid'=>$slideshowlistval[adminid]));?>" >[修改]</a><br>
										<a href="<?php echo U('pushresource',array('resourceid'=>$slideshowlistval[rid],'adminid'=>$slideshowlistval['adminid'],'isaudit'=>$slideshowlistval['isaudit']));?>" >[推送]</a><br>
										<a href="<?php echo U('delslideshow',array('id'=>$slideshowlistval[rid]));?>"  onclick="if(confirm('你确定要删除数据吗？')){ return true;}return false; ">[删除]</a><br>
										<?php if($slideshowlistval["isaudit"] == 1 ): ?>[已过审核]
										<?php else: ?> 
											<a href="<?php echo U('auditslideshow',array( 'id'=>$slideshowlistval[rid], 'rcode'=>$slideshowlistval[rcode] ));?>" >[去审核]</a><?php endif; ?>
